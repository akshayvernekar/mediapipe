...
namespace mediapipe {
    ...
    absl::Status PutTextOnImageCalculator::Process(CalculatorContext* cc) {
        ...
        if (!cc->Inputs().Tag(kInGesturesTag).IsEmpty()) {
            auto& gesture_strings =
                cc->Inputs().Tag(kInGesturesTag).Get<std::string>();
            ...
            cv::putText(input_frame_mat, gesture_strings, cv::Point(15, 70),
                        cv::FONT_HERSHEY_PLAIN, 3, cv::Scalar(255, 255, 0, 255),
                        4);
            cv::cvtColor(input_frame_mat, input_frame_mat, cv::COLOR_BGR2RGB);

            std::unique_ptr<ImageFrame> output_frame =
                absl::make_unique<ImageFrame>(
                    ImageFormat::SRGB, input_frame_mat.cols,
                    input_frame_mat.rows,
            ...
            cc->Outputs()
                .Tag(kOutImageTag)
                .AddPacket(cc->Inputs().Tag(kInImageTag).Value());
        }
        return absl::OkStatus();
    }
