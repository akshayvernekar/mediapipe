...
namespace mediapipe {
    ...
    class ZoomInCalculator : public CalculatorBase {
       public:
        ...
        absl::Status Process(CalculatorContext* cc) override {
            ...
            final_width = int(prev_detection.bbox().width() * zoom_scale);
            final_height = int(prev_detection.bbox().height() * zoom_scale);

            // Max zoom will be 4x or 25% of total image size
            final_width = std::max(final_width, frame_width / 4);
            final_height = std::max(final_height, frame_height / 4);
    ...
    REGISTER_CALCULATOR(ZoomInCalculator);
